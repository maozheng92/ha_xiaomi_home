...
- 米家集成是否支持所有的小米米家设备？
  ...
  米家集成目前支持大部分米家设备品类，但仍有一小部分设备品类（蓝牙、红外及虚拟设备）并不支持。小米万能遥控器 `chuangmi.ir.v2` 是例外：它会添加为一个 `remote` 实体。该设备没有 MIoT-Spec-V2，云端设备列表里通常也没有规格类型。安装此版本后，请到集成选项里更新一次设备列表，否则遥控器不会进入已导入列表。学习、发送和删除指令的用法与 Broadlink 集成相同，码值保存在本地。指令通过局域网本地控制发送。请开启局域网控制，并让 Home Assistant 与遥控器处于同一网络。如果局域网中存在中枢网关，局域网控制不会生效，此时无法使用该遥控器。
